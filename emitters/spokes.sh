...
emit_spoke_log_analytics() {
  ...
  local r="$1" spoke="$2"
  emit_log_analytics_common "$r" "Log Analytics (${spoke})"
}
...
emit_vm() {
  ...
  add_item "VM (${spoke}) ${size} (${os})" "hour" "$qty" "${p:-0}" "$(source_from_price "$p")" \
...
emit_custom() {
  ...
  notes="$(notes_from_cfg "$r")"
  add_item "Custom (${spoke}) - ${name}" "$unit" "$qty" "$up" "manual" "{}" "$notes"
}
